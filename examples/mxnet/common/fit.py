...
class LogValidationMetricsCallback(object):
    ...
    def __call__(self, param):
        ...
        for name, value in name_value:
            ...
            hvd.allreduce_(val_array, average=False, name=str(-1))
            val = val_array.asscalar()/hvd.size()
            print(hvd.rank(), value, val_array, val)
            logging.info('Epoch[%d] Validation-%s=%f', param.epoch, name, val)
...
def fit(args, network, data_loader, **kwargs):
    ...
    # TODO(@ctcyang): get rid of nworkers once average allreduce is in place
    # This is currently blocked by lack of GPU inplace div operator
    if 'rescale_grad' not in optimizer_params:
        optimizer_params['rescale_grad'] = 1.0/args.batch_size/hvd.size()
    ...
    #model.save_params('myfile'+str(hvd.rank()))
    (arg_params, aux_params) = model.get_params(copy_to_cpu=False)
    mx.ndarray.waitall()
    arg_params2 = hvd.broadcast_parameters(arg_params, root_rank=0)
    aux_params2 = hvd.broadcast_parameters(aux_params, root_rank=0)
    model.set_params(arg_params=arg_params2, aux_params=aux_params2)
