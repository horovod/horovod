...
#
# This model is an example of a computation-intensive model that achieves good
# accuracy on an image classification task.  It brings together distributed
# training concepts such as learning rate schedule adjustments with a warmup,
# randomized data reading, and checkpointing on the first worker only.
#
# Note: This model uses Keras native ImageDataGenerator and not the
# sophisticated preprocessing pipeline that is typically used to train
# state-of-the-art ResNet-50 model.  This results in ~0.5% increase in the
# top-1 validation error compared to the single-crop top-1 validation error from
...
if resume_from_epoch == 0:
    ...
    # Restore from a previous checkpoint, if initial epoch is specified.
...
callbacks = [
    ...
    # Horovod: after the warmup reduce learning rate by 10 on the 30th, 60th
    # and 80th epochs.
    hvd.callbacks.LearningRateScheduleCallback(start_epoch=warmup_epochs,
                                               end_epoch=30, multiplier=1.),
    hvd.callbacks.LearningRateScheduleCallback(start_epoch=30, end_epoch=60,
                                               multiplier=1e-1),
    hvd.callbacks.LearningRateScheduleCallback(start_epoch=60, end_epoch=80,
                                               multiplier=1e-2),
    hvd.callbacks.LearningRateScheduleCallback(start_epoch=80, multiplier=1e-3),
]

# Horovod: save checkpoints only on the first worker to prevent other workers
...
# Train the model. The training will randomly sample 1 / N batches of training
# data and 3 / N batches of validation data on every worker, where N is the
# number of workers. Over-sampling of validation data helps to increase
# probability that every validation example will be evaluated.
