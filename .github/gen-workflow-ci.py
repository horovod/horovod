...
def main():
    ...
    def trigger_buildkite_job(name: str, needs: List[str]) -> str:
        return (f'  {name}:\n'
                ...
                f'\n'
                f'    steps:\n'
                f'      - name: Cancel running Buildkite builds\n'
                f'        continue-on-error: true\n'
                f'        env:\n'
                f'          branch: ${{{{ github.event.pull_request.head.label }}}}\n'
                f'        run: |\n'
                f'          echo "::group::Identify running builds"\n'
                f'          curl -s -H "Authorization: Bearer ${{{{ secrets.BUILDKITE_TOKEN }}}}" "https://api.buildkite.com/v2/organizations/horovod/pipelines/horovod/builds?branch=${{branch/#horovod:/}}&state[]=scheduled&state[]=running" > buildkite.running-builds.json\n'
                f'          ids=$(jq -r .[].id buildkite.running-builds.json)\n'
                f'          echo "::endgroup::"\n'
                f'\n'
                f'          echo "::group::Cancelling running builds"\n'
                f'          for id in ${{ids}}\n'
                f'          do\n'
                f'            echo -n "- Build $id: "\n'
                f'            curl -s -H "Authorization: Bearer ${{{{ secrets.BUILDKITE_TOKEN }}}}" -X PUT "https://api.buildkite.com/v2/organizations/horovod/pipelines/horovod/builds/${{id}}/cancel" | jq -r .state\n'
                f'          done\n'
                f'          echo "::endgroup::"\n'
                f'        shell: bash\n'
                f'\n'
