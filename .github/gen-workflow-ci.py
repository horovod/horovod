...
def main():
    ...
    def build_and_test_images(name: str,
        ...
        return (f'  {name}:\n'
                ...
                f'              build:\n'
                f'                cache_from:\n'
                f'                  - ${{{{ steps.ecr.outputs.registry }}}}:horovod-${{{{ matrix.image }}}}-latest\n'
                f'          EOF\n'
                ...
                f'        id: build\n'
                f'        run: |\n'
                f'          if [ ! -e docker-compose.test.override.yml ]; then touch docker-compose.test.override.yml; fi\n'
                f'          .github/timeout-and-retry.sh ${{{{ matrix.build_timeout }}}}m 3 10 docker-compose -f docker-compose.test.yml -f docker-compose.test.override.yml build --pull ${{{{ matrix.image }}}}\n'
                f'\n' +
