...
def main():
    ...
    def build_and_test_macos(id: str, name: str, needs: List[str], attempts: int = 3) -> str:
        ...
        return (f'  {id}:\n'
                ...
                f'          submodules: recursive\n'
                f'\n' +
                '\n'.join([f'      - name: Build and Test [attempt {attempt} of {attempts}]\n'
                           f'        id: build-and-test-{attempt}\n'
                           f'        continue-on-error: {"true" if attempt < attempts else "false"}\n'
                           f'        if: always() && {"true" if attempt == 1 else f"steps.build-and-test-{attempt-1}.outcome == {failure}"}\n'
                           f'        env:\n'
                           ...
                           f'\n'
                           f'        run: |\n'
                           f'          brew install -f openmpi cmake libuv pyenv\n'
                ...
                f'          path: |\n' +
                '\n'.join([f'            ${{{{ steps.build-and-test-{attempt}.outputs.artifacts-path }}}}'
                           for attempt in range(1, attempts+1)]))
