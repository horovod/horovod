...
def main():
    ...
    def build_and_test_macos(name: str, needs: List[str]) -> str:
        return (f'  {name}:\n'
                f'    name: "Build and Test (${{{{ matrix.name }}}}-macos)"\n'
                f'    needs: [{", ".join(needs)}]\n'
                ...
                f'        include:\n'
                f''
                f'          - name: test-cpu-openmpi-py3_7-tf1_15_5-keras2_2_4-torch1_2_0-mxnet1_5_0\n'
                ...
                f'            MXNET: 1.5.0\n'
                f'\n'
                f'          - name: test-cpu-gloo-py3_8-tf2_2_0-keras2_3_1-torch1_5_0-mxnet1_5_0\n'
                ...
                f'            MXNET: 1.5.0\n'
                f'\n'
                f'          - name: test-openmpi-cpu-gloo-py3_8-tf2_3_0-keras2_3_1-torch1_6_0-mxnet1_5_0\n'
                f'            HOROVOD_WITH_MPI: 1\n'
                f'            HOROVOD_WITH_GLOO: 1\n'
                f'            TENSORFLOW: 2.3.0\n'
                f'            KERAS: 2.3.1\n'
                ...
                f'          echo pytest -v --capture=no --continue-on-collection-errors --junit-xml=artifacts/${{{{ matrix.image }}}}-macos/junit.\$1.\${{HOROVOD_RANK:-\${{OMPI_COMM_WORLD_RANK:-\${{PMI_RANK}}}}}}.\$2.xml \${{@:2}} > pytest.sh\n'
                f'          chmod u+x pytest.sh\n'
                f'          cat pytest.sh\n'
                f'          which horovodrun\n'
                f'\n'
                f'      - name: Run Tests\n'
                f'        run: cd test && (ls parallel/test_*.py | xargs -n 1 horovodrun -np 2 ./pytest.sh)\n'
                f'\n'
                f'      - name: Upload Test Results\n'
                f'        uses: actions/upload-artifact@v2\n'
                f'        if: always()\n'
