...
run_mpi_pytest() {
  ...
  local exclude_standalone_test="| sed 's/test_spark.py//g' | sed 's/test_run.py//g' | sed 's/test*_ray.py//g'"
...
run_gloo_pytest() {
  ...
  # Spark and Run test does not need to be executed with horovodrun, but we still run it below.
  local exclude_standalone_test="| sed 's/test_spark.py//g' | sed 's/test_run.py//g' | sed 's/test*_ray.py//g'"
  local standalone_tests="test_spark.py test_run.py test*_ray.p"
