  ...
  if [[ ${test} != *"mpich"* && ${test} != *"oneccl"* ]]; then
    if [[ ${test} == *"tf2_"* ]] || [[ ${test} == *"tfhead"* ]]; then
      run_test "${test}" "${queue}" \
        ...
        "bash -c \"bash -c 'while true; do which nvidia-smi || exit; nvidia-smi; sleep 1; done' & cd /horovod/examples/spark/tensorflow2; spark-submit --master \\\"local[2]\\\" \\\"/horovod/horovod/spark/tensorflow/compute_worker.py\\\" /tmp/compute.json & OMP_NUM_THREADS=1 /spark_env.sh spark-submit --master \\\"local[2]\\\" --py-files tensorflow2_mnist_data_service_train_fn_compute_side_dispatcher.py,tensorflow2_mnist_data_service_train_fn_training_side_dispatcher.py tensorflow2_mnist_data_service.py /tmp/compute.json\""
    fi
  fi
}
