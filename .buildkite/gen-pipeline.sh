...
run_mpi_pytest() {
  ...
  run_test "${test}" "${queue}" \
    ...
    "bash -c \"${oneccl_env} cd /horovod/test && (echo test_*.py ${exclude_keras} ${excluded_tests} ${exclude_standalone_test} | xargs -n 1 \\\$(cat /mpirun_command) bash -c 'pytest -v --capture=no --junit-xml=/artifacts/junit.mpi.\${HOROVOD_RANK:-\${OMPI_COMM_WORLD_RANK:-\${PMI_RANK}}}.xml') && pytest --forked -v --capture=fd --junit-xml=/artifacts/junit.mpi.standalone.xml ${standalone_tests}\"" \
    10

  run_test "${test}" "${queue}" \
    ":pytest: Run Cluster PyTests (${test})" \
    "bash -c \"${oneccl_env} /etc/init.d/ssh start && cd /horovod/test/integration && pytest --forked -v --capture=fd --junit-xml=/artifacts/junit.mpi.static.xml test_static_run.py\""
}
...
run_mpi_integration() {
  ...
  if [[ ${test} != *"mpich"* ]] && [[ ${test} != *"oneccl"* ]]; then
    ...
    run_test "${test}" "${queue}" \
      ...
      "bash -c \"cd /horovod/test && pytest -v --capture=no --junit-xml=/artifacts/junit.mpi.integration.xml test_interactiverun.py\""
  fi
...
run_gloo_pytest() {
  ...
  run_test "${test}" "${queue}" \
    ":pytest: Run PyTests (${test})" \
    "bash -c \"cd /horovod/test && (echo test_*.py ${exclude_keras} ${excluded_tests} ${exclude_standalone_test} | xargs -n 1 horovodrun -np 2 -H localhost:2 --gloo bash -c 'pytest -v --capture=no --junit-xml=/artifacts/junit.gloo.\${HOROVOD_RANK:-\${OMPI_COMM_WORLD_RANK:-\${PMI_RANK}}}.xml') && pytest --forked -v --capture=fd --junit-xml=/artifacts/junit.gloo.standalone.xml ${standalone_tests}\"" \
    10

  run_test "${test}" "${queue}" \
    ":pytest: Run Cluster PyTests (${test})" \
    "bash -c \"/etc/init.d/ssh start && cd /horovod/test/integration && pytest --forked -v --capture=fd --junit-xml=/artifacts/junit.gloo.static.xml test_static_run.py\""
}
...
run_gloo_integration() {
  ...
  run_test "${test}" "${queue}" \
    ...
    "bash -c \"cd /horovod/test/integration && HOROVOD_LOG_LEVEL=DEBUG pytest --forked -v --log-cli-level 10 --log-cli-format '[%(asctime)-15s %(levelname)s %(filename)s:%(lineno)d %(funcName)s()] %(message)s' --capture=no --junit-xml=/artifacts/junit.gloo.elastic.xml test_elastic_torch.py ${elastic_tensorflow}\""

  run_test "${test}" "${queue}" \
    ":factory: Elastic Spark TensorFlow Tests (${test})" \
    "bash -c \"cd /horovod/test/integration && SPARK_HOME=/spark SPARK_DRIVER_MEM=512m HOROVOD_LOG_LEVEL=DEBUG pytest --forked -v --log-cli-level 10 --log-cli-format '[%(asctime)-15s %(levelname)s %(filename)s:%(lineno)d %(funcName)s()] %(message)s' --capture=no --junit-xml=/artifacts/junit.gloo.elastic.spark.tf.xml ${elastic_spark_tensorflow}\"" \
    15

  run_test "${test}" "${queue}" \
    ":factory: Elastic Spark Torch Tests (${test})" \
    "bash -c \"cd /horovod/test/integration && SPARK_HOME=/spark SPARK_DRIVER_MEM=512m HOROVOD_LOG_LEVEL=DEBUG pytest --forked -v --log-cli-level 10 --log-cli-format '[%(asctime)-15s %(levelname)s %(filename)s:%(lineno)d %(funcName)s()] %(message)s' --capture=no --junit-xml=/artifacts/junit.gloo.elastic.spark.torch.xml test_elastic_spark_torch.py\"" \
    15
    ...
    if [[ ${test} != *"tf2"* && ${test} != *"tfhead"* ]]; then
      ...
      if [[ ${queue} != *gpu* ]]; then
        run_test "${test}" "${queue}" \
          ...
          "bash -c \"cd /horovod/test && pytest --forked -v --capture=no --junit-xml=/artifacts/junit.spark.integration.xml test_spark_keras.py test_spark_torch.py\""
      fi
    fi
