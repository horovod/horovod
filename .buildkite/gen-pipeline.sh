...
run_all() {
  ...
  if [[ ${test} == *"tf2_"* ]]; then
    ...
    run_test "${test}" "${queue}" \
      ":muscle: Test TensorFlow 2.0 Keras MNIST with DistributionStrategy (${test})" \
      "bash -c \"\\\$(cat /mpirun_command) python /horovod/examples/tensorflow2_keras_mnist_strategy.py\""
  fi
}
