...
run_mpi_pytest() {
  ...
  run_test "${test}" "${queue}" \
    ...
    "bash -c \"${oneccl_env} cd /horovod/test && (echo test_*.py ${exclude_keras} ${excluded_tests} ${exclude_standalone_test} | xargs -n 1 \\\$(cat /mpirun_command) pytest -v --capture=no) && pytest --forked -v --capture=no ${standalone_tests} | ts '[%Y-%m-%d %H:%M:%S]' && cd integration && /etc/init.d/ssh start && pytest --forked -v --capture=no test_run.py\""
}
...
run_gloo_pytest() {
  ...
  run_test "${test}" "${queue}" \
    ":pytest: Run PyTests (${test})" \
    "bash -c \"cd /horovod/test && (echo test_*.py ${exclude_keras} ${excluded_tests} ${exclude_standalone_test} | xargs -n 1 horovodrun -np 2 -H localhost:2 --gloo pytest -v --capture=no) && pytest --forked -v --capture=no ${standalone_tests} | ts '[%Y-%m-%d %H:%M:%S]' && cd integration && /etc/init.d/ssh start && pytest --forked -v --capture=no test_run.py\""
}
