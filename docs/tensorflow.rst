...
4. Wrap the optimizer in ``hvd.DistributedOptimizer``.
   ...
   **Note:** For model parallel use cases there are local variables (layers) that their gradients need not to be synced (by allreduce or allgather). For such use cases, you can use the ``hvd.PartialDistributedOptimizer`` and pass the local layers to this API in order to register their local variables. Additionally, when using ``tf.GradientTape``, wrap the tape in ``hvd.PartialDistributedGradientTape`` instead of ``DistributedGradientTape`` and pass the local layers to it in order to register their local variables.
