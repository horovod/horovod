...
pair that allows `passwordless authentication <http://www.linuxproblem.org/art_9.html>`__. Additionally a ``config`` is added, in which all workers are defined. An example of a ``config`` can be seen in :ref:`Horovod in Docker:Run containers with different ports`.
