...
bool MPIController::RecvReadyTensors(
    ...
    std::shared_ptr<MessageTable> message_table, Timeline& timeline) {
  ...
  for (int i = 1; i < size_; ++i) {
    ...
    for (auto& received_message : received_message_list.requests()) {
      ...
      bool reduce = IncrementTensorCount(message_table, received_message, size_,
                                         timeline);
...
void MPIController::SynchronizeParameters(ParameterManager& para_manager) {
  ParameterManager::Params param;
  if (is_coordinator_) {
    para_manager.GetParam(param);
  }
  ...
  if (not is_coordinator_) {
    para_manager.UpdataParam(param);
  }
  para_manager.Reset();
}
