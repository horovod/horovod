...
void BatchedD2DMemcpyHipImpl(BatchedD2DParams& params, int num_copies, hipStream_t stream)
{
...
void ScaleBufferHipImpl(const void* fused_input_data, void* buffer_data, const int64_t num_elements, double scale_factor,
  ...
  switch (dtype) {
    ...
    default:
      throw std::logic_error("Type " + DataType_Name(dtype) +
                             " not supported by ScaleBufferHipImpl.");
  }
}
...
void BatchedScaledD2DMemcpyHipImpl(BatchedD2DParams& params, int num_copies, double scale_factor,
                                    DataType dtype, hipStream_t stream) {
  ...
  switch (dtype) {
   ...
   default:
     throw std::logic_error("Type " + DataType_Name(dtype) +
                            " not supported by BatchedScaledD2DMemcpyHipImpl.");
  }
}
