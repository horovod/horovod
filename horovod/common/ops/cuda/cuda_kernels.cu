...
#define ADASUM_KERNEL_THREADS_PER_BLOCK 128
...
void CudaDotProductKernel(int count, const T* a, const T* b, TACC* out) {
	__shared__ TACC normsq_a[ADASUM_KERNEL_THREADS_PER_BLOCK];
	__shared__ TACC normsq_b[ADASUM_KERNEL_THREADS_PER_BLOCK];
	__shared__ TACC dot[ADASUM_KERNEL_THREADS_PER_BLOCK];
	int index = threadIdx.x + blockIdx.x * blockDim.x;
	if (index < count){
	...
	if (0 == threadIdx.x) {
		...
		TACC normsq_b_sum = 0;
		TACC dot_sum = 0;
		for(int i = 0; i < ADASUM_KERNEL_THREADS_PER_BLOCK; i++){
...
void CudaSingleAdasumImpl(int count, double* device_a, const double* device_b,
	...
	CudaDotProductKernel<<<(count+ADASUM_KERNEL_THREADS_PER_BLOCK-1)/ADASUM_KERNEL_THREADS_PER_BLOCK,
		ADASUM_KERNEL_THREADS_PER_BLOCK, 0, stream>>>(count, device_a, device_b, device_vals);
	ErrorCheck("CudaDotProductKernel(double)", cudaGetLastError());
	CudaSingleAdasumKernel<<<(count+ADASUM_KERNEL_THREADS_PER_BLOCK-1)/ADASUM_KERNEL_THREADS_PER_BLOCK, ADASUM_KERNEL_THREADS_PER_BLOCK, 0, stream>>>(count, device_a, device_b,
		device_vals);
}
...
void CudaSingleAdasumImpl(int count, uint16_t* device_a, const uint16_t* device_b,
	...
	CudaDotProductKernel<<<(count+ADASUM_KERNEL_THREADS_PER_BLOCK-1)/ADASUM_KERNEL_THREADS_PER_BLOCK,
		ADASUM_KERNEL_THREADS_PER_BLOCK, 0, stream>>>(count, (__half*) device_a, (__half*) device_b, device_vals);
	ErrorCheck("CudaDotProductKernel(fp16)", cudaGetLastError());
	CudaSingleAdasumKernel<<<(count+ADASUM_KERNEL_THREADS_PER_BLOCK-1)/ADASUM_KERNEL_THREADS_PER_BLOCK, ADASUM_KERNEL_THREADS_PER_BLOCK, 0, stream>>>(count, (__half*)device_a, (__half*)device_b,
		device_vals);
}
...
void CudaDotProductImpl(int count, const double* device_a, const double* device_b,
	...
	CudaDotProductKernel<<<(count+ADASUM_KERNEL_THREADS_PER_BLOCK-1)/ADASUM_KERNEL_THREADS_PER_BLOCK,
		ADASUM_KERNEL_THREADS_PER_BLOCK>>>(count, device_a, device_b, device_vals);
	ErrorCheck("CudaDotProductKernel(double)", cudaGetLastError());
...
void CudaDotProductImpl(int count, const uint16_t* device_a, const uint16_t* device_b,
	...
	CudaDotProductKernel<<<(count+ADASUM_KERNEL_THREADS_PER_BLOCK-1)/ADASUM_KERNEL_THREADS_PER_BLOCK,
		ADASUM_KERNEL_THREADS_PER_BLOCK>>>(count, (__half*) device_a, (__half*) device_b, device_vals);
	ErrorCheck("CudaDotProductKernel(fp16)", cudaGetLastError());
...
void CudaScaleAddImpl(int count, double* a_device, const double* b_device, double host_a_coeff, double host_b_coeff) {
	CudaScaleAddKernel<<<(count+ADASUM_KERNEL_THREADS_PER_BLOCK-1)/ADASUM_KERNEL_THREADS_PER_BLOCK, ADASUM_KERNEL_THREADS_PER_BLOCK>>>(count, a_device, b_device,
...
void CudaScaleAddImpl(int count, uint16_t* a_device, const uint16_t* b_device, double host_a_coeff, double host_b_coeff) {
	CudaScaleAddKernel<<<(count+ADASUM_KERNEL_THREADS_PER_BLOCK-1)/ADASUM_KERNEL_THREADS_PER_BLOCK, ADASUM_KERNEL_THREADS_PER_BLOCK>>>(count, (__half*)a_device, (__half*)b_device,
		host_a_coeff, host_b_coeff);
}
