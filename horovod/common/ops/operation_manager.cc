...
OperationManager::OperationManager(ParameterManager* param_manager,
    ...
    : param_manager_(param_manager),
      ...
      parasail_ops_(std::move(parasail_ops)),
...
Status OperationManager::ExecuteParasail(std::vector<TensorTableEntry>& entries,
                                          const Response& response) const {
  for (auto& op : parasail_ops_) {
    if (op->Enabled(*param_manager_, entries, response)) {
      return op->Execute(entries, response);
    }
  }
  throw std::logic_error("No parasail operation enabled");
}
...
Status OperationManager::ExecuteOperation(std::vector<TensorTableEntry>& entries,
  ...
  } else if (response.response_type() == Response::PARASAIL) {
    return ExecuteParasail(entries, response);
