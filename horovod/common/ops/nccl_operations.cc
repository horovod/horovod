...
// step1: copy data to fusion buffer, ncclReduceScatter/ncclReduce, copy data to
// CPU step2: use MPI do allreduce in CPU step3: copy data from CPU back to GPU,
// ncclAllGather/ncclBcast, copy data back to tensor
//
// set this 3 step is: a, b, c and suppose we need n allReduce and the step in
// NCCLHierarchicalAllreduce is serial, so we can think the whole process of
// NCCLHierarchicalAllreduce is below:
//
// a1, b1, c1, a2, b2, c2, ..., an, bn, cn
//
// So the total time is: sum[a1..n] + sum[b1..n] + sum[c1..n]
//
// in ParallelNCCLHierarchicalAllreduce use 3 thread to do the same thing
// (horovod background thread, MPI thread, End thread) than the whole process
// can be
//
// a1, a2, ..., an
//     b1, b2, ..., bn
...
Status ParallelNCCLHierarchicalAllreduce::Execute(
  ...
  auto stream = parallel_cuda_context_->streams[first_entry.device];
  ...
  if (global_state_->timeline.Initialized()) {
    parallel_cuda_context_->RecordEvent(event_queue, QUEUE, stream);
  }

  // step1: in this main thread will do ncclReduceScatter/ncclReduce, copy data
  // to CPU and will use stream and nccl_comm to do the task do the same thing
  // like NCCLHierarchicalAllreduce
  const void* fused_input_data;
  ...
  if (entries.size() > 1) {
    ...
    MemcpyInFusionBuffer(entries, fused_input_data, fusion_buffer,
                         fusion_buffer_len);

    if (global_state_->timeline.Initialized()) {
      parallel_cuda_context_->RecordEvent(event_queue, MEMCPY_IN_FUSION_BUFFER,
                                          stream);
    }
  } else {
    // reuse tensor data
  ...
  int element_size =
      parallel_mpi_context_->GetMPITypeSize(first_entry.tensor->dtype());
  ...
  int64_t num_elements_per_rank =
      global_state_->is_homogeneous
          ? total_num_elements / global_state_->local_size
          : 0;

  size_t buffer_len_per_rank = num_elements_per_rank * element_size;

  if (num_elements_per_rank > 0) {
    void* fusion_buffer_at_rank_offset =
        (uint8_t*)fusion_buffer +
        global_state_->local_rank * buffer_len_per_rank;

    auto nccl_result = ncclReduceScatter(
        fused_input_data, fusion_buffer_at_rank_offset,
    ...
    if (global_state_->timeline.Initialized()) {
      parallel_cuda_context_->RecordEvent(event_queue, NCCL_REDUCESCATTER,
                                          stream);
    }
  }
  ...
  if (global_state_->is_homogeneous || is_root_rank) {
    ...
    // copy data to CPU
    timeline.ActivityStartAll(entries, MEMCPY_IN_HOST_BUFFER);
    parallel_cuda_context_->ErrorCheck(
        "cudaMemcpyAsync",
        cudaMemcpyAsync(host_buffer, fusion_buffer_at_rank_offset,
                        host_buffer_len, cudaMemcpyDeviceToHost, stream));
    timeline.ActivityEndAll(entries);
  }

  // at here have copy data to CPU buffer, now need put the MPI allreduce in MPI
  // thread before put task need get all resource in this thread to make sure
  // thread safe end_fusion_buffer is the GPU buffer used in end thread, if
  // entries.size() > 1 we will create a new GPU memory if not. it will reuse
  // the tensor output buffer
  void* end_fusion_buffer = nullptr;

  if (entries.size() > 1) {
    auto& end_buffer = global_state_->fusion_buffer.GetEndBuffer(
        first_entry.device, first_entry.context->framework());

    end_fusion_buffer =
        const_cast<void*>(end_buffer->AccessData(first_entry.context));
  } else {
    end_fusion_buffer = (void*)first_entry.output->data();
  }

  // put task in MPI thread
  mpi_queue_.enqueue([this, entries, host_buffer, end_fusion_buffer, end_stream,
                      end_nccl_comm, num_elements_per_rank,
                      num_elements_remaining, element_size] {
    this->ExecuteMPIAllReduce(entries, host_buffer, end_fusion_buffer,
                              end_stream, end_nccl_comm, num_elements_per_rank,
                              num_elements_remaining, element_size);
  });
...
Status ParallelNCCLHierarchicalAllreduce::ExecuteMPIAllReduce(std::vector<TensorTableEntry> entries, 
  ...
  if (global_state_->is_homogeneous || is_root_rank) {
    ...
    int op = MPI_Allreduce(
        MPI_IN_PLACE, host_buffer, (int)num_elements,
  ...
  end_queue_.enqueue([this, entries, host_buffer, end_fusion_buffer, end_stream,
                      end_nccl_comm, num_elements_per_rank,
                      num_elements_remaining, element_size] {
    this->ExecuteEnd(entries, host_buffer, end_fusion_buffer, end_stream,
                     end_nccl_comm, num_elements_per_rank,
                     num_elements_remaining, element_size);
  });
...
Status ParallelNCCLHierarchicalAllreduce::ExecuteEnd(std::vector<TensorTableEntry> entries, 
  ...
  parallel_cuda_context_->ErrorCheck("cudaSetDevice", cuda_result);
  ...
  if (global_state_->is_homogeneous || is_root_rank) {
    ...
    void* end_fusion_buffer_at_offset =
        (uint8_t*)end_fusion_buffer +
  ...
  if (num_elements_per_rank > 0) {
    ...
    if (global_state_->timeline.Initialized()) {
      parallel_cuda_context_->RecordEvent(end_event_queue, NCCL_ALLGATHER,
                                          end_stream);
    }
  }

  if (num_elements_remaining > 0) {
    void* end_fusion_buffer_remainder =
        (uint8_t*)end_fusion_buffer +
        global_state_->local_size * num_elements_per_rank * element_size;
    ...
    if (global_state_->timeline.Initialized()) {
      parallel_cuda_context_->RecordEvent(end_event_queue, NCCL_BCAST,
                                          end_stream);
    }
  }
  ...
  if (entries.size() > 1) {
    ...
    if (global_state_->timeline.Initialized()) {
      parallel_cuda_context_->RecordEvent(end_event_queue,
                                          MEMCPY_OUT_FUSION_BUFFER, end_stream);
    }
  }
