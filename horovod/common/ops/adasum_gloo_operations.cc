...
#include "adasum_gloo_operations.h"

namespace horovod {
namespace common {
AdasumGlooAllreduceOp::AdasumGlooAllreduceOp(GlooContext* gloo_context,
                                             HorovodGlobalState* global_state)
    : AdasumGloo(gloo_context, global_state), AllreduceOp(global_state) {}

bool AdasumGlooAllreduceOp::Enabled(const ParameterManager& param_manager,
                                    const std::vector<TensorTableEntry>& entries,
                                    const Response& response) const {
  return true;
}

Status AdasumGlooAllreduceOp::Execute(std::vector<TensorTableEntry>& entries,
  ...
  // Do allreduce.
  timeline.ActivityStartAll(entries, GLOO_ADASUM_ALLREDUCE);
