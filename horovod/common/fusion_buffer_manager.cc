...
Status FusionBufferManager::InitializeBuffer(
    ...
    int stream_id, std::function<void()> on_start_init,
    std::function<void()> on_end_init) {
  auto& elem = tensor_fusion_buffers_[std::make_tuple(
      device, context->framework(), stream_id)];
...
FusionBufferManager::GetBuffer(int device, Framework framework, int stream_id) {
  return tensor_fusion_buffers_[std::make_tuple(device, framework, stream_id)]
      .first;
}
