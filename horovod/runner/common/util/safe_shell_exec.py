...
def forward_stream(src_stream, dst_stream, prefix, index, hide_timestamp):
    def prepend_context(line, rank, prefix):
        localtime = time.asctime(time.localtime(time.time())) if not hide_timestamp else ''
...
def execute(command, env=None, stdout=None, stderr=None, index=None, events=None, hide_timestamp=False):
    ...
    stdout_fwd = in_thread(target=forward_stream, args=(stdout_r, stdout, 'stdout', index, hide_timestamp))
    stderr_fwd = in_thread(target=forward_stream, args=(stderr_r, stderr, 'stderr', index, hide_timestamp))
