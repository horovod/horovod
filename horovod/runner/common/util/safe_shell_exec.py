...
def forward_stream(src_stream, dst_stream, prefix, index, timestamp_output):
    def prepend_context(line, rank, prefix):
        localtime = time.asctime(time.localtime(time.time())) if timestamp_output else ''
...
def execute(command, env=None, stdout=None, stderr=None, index=None, events=None, timestamp_output=False):
    ...
    stdout_fwd = in_thread(target=forward_stream, args=(stdout_r, stdout, 'stdout', index, timestamp_output))
    stderr_fwd = in_thread(target=forward_stream, args=(stderr_r, stderr, 'stderr', index, timestamp_output))
